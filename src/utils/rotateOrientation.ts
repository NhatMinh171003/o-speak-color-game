...
function getVoicePriority(key: string): number {
    ...
    // SpeakScene voices: intro, speaking, line prompts
    if (
        key === 'intro-speak' ||
        key === 'intro-voice' ||
        key === 'voice-speaking' ||
        key.startsWith('begin-line')
    ) {
        return 3;
    }
    // Scene2 voices
    if (key === 'voice_intro_s2' || key === 'hint' || key === 'instruction') {
        return 3;
    }
    if (
        key === 'voice_complete' ||
...
export function playVoiceLocked(
    ...
    // Auto-clear currentVoiceKey khi sound phát xong để không block voice tiếp theo
